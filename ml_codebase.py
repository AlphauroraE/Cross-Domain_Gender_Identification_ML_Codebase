...
# These are our ml models, with random_state = 0 for replicability
classifier = {
...
experiment_info = {'train_info': '', 'test_info': '', 'relative_info': '', 'posorvel_info': '', 'subsample_info': ''}
...
def userIDConventionFAST(filename = None,filepath = None):
    ...
    for i in range(len(filename)):
        if (filename[i] == "F" and filename[i+1] == "A" and filename[i+2] == "S"):
            start_ind = i+3
...
def runExperimentsScene(train_info, test1_info, test2_info, test3_info, posorvel, experiment_info):
    ...
    if (posorvel == "p"):
        featsList = getScenePosList()
        print(len(featsList), "Position Features")
    elif (posorvel == "v"):
        featsList = getSceneVelList()
        print(len(featsList), "Velocity Features")
    ...
    experiment_info1 = {'train_info': train_set, 'test_info': test1_set, 'relative_info': experiment_info['relative_info'], 'posorvel_info': experiment_info['posorvel_info'], 'subsample_info': experiment_info['subsample_info']}
    experiment_info2 = {'train_info': train_set, 'test_info': test2_set, 'relative_info': experiment_info['relative_info'], 'posorvel_info': experiment_info['posorvel_info'], 'subsample_info': experiment_info['subsample_info']}
    experiment_info3 = {'train_info': train_set, 'test_info': test3_set, 'relative_info': experiment_info['relative_info'], 'posorvel_info': experiment_info['posorvel_info'], 'subsample_info': experiment_info['subsample_info']}



    # Folder names
    if (experiment_info['subsample_info'] != ""):
        folder1_name = f"{train_set}-{test1_set}_{experiment_info['relative_info']}_{experiment_info['posorvel_info']}_{experiment_info['subsample_info']}"
        folder2_name = f"{train_set}-{test2_set}_{experiment_info['relative_info']}_{experiment_info['posorvel_info']}_{experiment_info['subsample_info']}"
        folder3_name = f"{train_set}-{test3_set}_{experiment_info['relative_info']}_{experiment_info['posorvel_info']}_{experiment_info['subsample_info']}"
    else:
        folder1_name = f"{train_set}-{test1_set}_{experiment_info['relative_info']}_{experiment_info['posorvel_info']}"
        folder2_name = f"{train_set}-{test2_set}_{experiment_info['relative_info']}_{experiment_info['posorvel_info']}"
        folder3_name = f"{train_set}-{test3_set}_{experiment_info['relative_info']}_{experiment_info['posorvel_info']}"
    ...
    if (experiment_info['subsample_info'] != ""):
        score_name_knn_1 = f"{train_set}-{test1_set}_kNN_{experiment_info['relative_info']}_{experiment_info['posorvel_info']}_{experiment_info['subsample_info']}"
        score_name_knn_2 = f"{train_set}-{test2_set}_kNN_{experiment_info['relative_info']}_{experiment_info['posorvel_info']}_{experiment_info['subsample_info']}"
        score_name_knn_3 = f"{train_set}-{test3_set}_kNN_{experiment_info['relative_info']}_{experiment_info['posorvel_info']}_{experiment_info['subsample_info']}"

        score_name_rf_1 = f"{train_set}-{test1_set}_RF_{experiment_info['relative_info']}_{experiment_info['posorvel_info']}_{experiment_info['subsample_info']}"
        score_name_rf_2 = f"{train_set}-{test2_set}_RF_{experiment_info['relative_info']}_{experiment_info['posorvel_info']}_{experiment_info['subsample_info']}"
        score_name_rf_3 = f"{train_set}-{test3_set}_RF_{experiment_info['relative_info']}_{experiment_info['posorvel_info']}_{experiment_info['subsample_info']}"
    
        score_name_gbm_1 = f"{train_set}-{test1_set}_GBM_{experiment_info['relative_info']}_{experiment_info['posorvel_info']}_{experiment_info['subsample_info']}"
        score_name_gbm_2 = f"{train_set}-{test2_set}_GBM_{experiment_info['relative_info']}_{experiment_info['posorvel_info']}_{experiment_info['subsample_info']}"
        score_name_gbm_3 = f"{train_set}-{test3_set}_GBM_{experiment_info['relative_info']}_{experiment_info['posorvel_info']}_{experiment_info['subsample_info']}"
    else:
        score_name_knn_1 = f"{train_set}-{test1_set}_kNN_{experiment_info['relative_info']}_{experiment_info['posorvel_info']}"
        score_name_knn_2 = f"{train_set}-{test2_set}_kNN_{experiment_info['relative_info']}_{experiment_info['posorvel_info']}"
        score_name_knn_3 = f"{train_set}-{test3_set}_kNN_{experiment_info['relative_info']}_{experiment_info['posorvel_info']}"

        score_name_rf_1 = f"{train_set}-{test1_set}_RF_{experiment_info['relative_info']}_{experiment_info['posorvel_info']}"
        score_name_rf_2 = f"{train_set}-{test2_set}_RF_{experiment_info['relative_info']}_{experiment_info['posorvel_info']}"
        score_name_rf_3 = f"{train_set}-{test3_set}_RF_{experiment_info['relative_info']}_{experiment_info['posorvel_info']}"
    
        score_name_gbm_1 = f"{train_set}-{test1_set}_GBM_{experiment_info['relative_info']}_{experiment_info['posorvel_info']}"
        score_name_gbm_2 = f"{train_set}-{test2_set}_GBM_{experiment_info['relative_info']}_{experiment_info['posorvel_info']}"
        score_name_gbm_3 = f"{train_set}-{test3_set}_GBM_{experiment_info['relative_info']}_{experiment_info['posorvel_info']}"
...
def runExperimentsHead(train_info, test1_info, test2_info, test3_info, posorvel, experiment_info):
    # train_info = {'raw': , 'path': , 'fora': , 'set': }
    # test1_info = {'raw': , 'path': , 'fora': , 'set': , 'filename': }
    # experiment_info = {'relative_info': , 'posorvel_info': , 'subsample_info': }
    import datetime
    ...
    # Position and Velocity Features
    if (posorvel == "p"):
        featsList = getHeadPosList()
        print(len(featsList), "Position Features")
    elif (posorvel == "v"):
        featsList = getHeadVelList()
        print(len(featsList), "Velocity Features")
    ...
    # For each feature set:
    for j in range(len(featsList)):
        # Omit certain experiments due to head-relative nature
        if (j in [3,10,17,28,29,30,31]):
            # Insert blank space to tables
            # KNN 1
            score49ToTable(0,int(j),score_df_knn_1)
            score49ToTable(0,int(j),f1_df_knn_1)
            score49ToTable(0,int(j),prec_df_knn_1)
            score49ToTable(0,int(j),recall_df_knn_1)

            # KNN 2
            score49ToTable(0,int(j),score_df_knn_2)
            score49ToTable(0,int(j),f1_df_knn_2)
            score49ToTable(0,int(j),prec_df_knn_2)
            score49ToTable(0,int(j),recall_df_knn_2)

            # KNN 3
            score49ToTable(0,int(j),score_df_knn_3)
            score49ToTable(0,int(j),f1_df_knn_3)
            score49ToTable(0,int(j),prec_df_knn_3)
            score49ToTable(0,int(j),recall_df_knn_3)


            # RF 1
            score49ToTable(0,int(j),score_df_rf_1)
            score49ToTable(0,int(j),f1_df_rf_1)
            score49ToTable(0,int(j),prec_df_rf_1)
            score49ToTable(0,int(j),recall_df_rf_1)

            # RF 2
            score49ToTable(0,int(j),score_df_rf_2)
            score49ToTable(0,int(j),f1_df_rf_2)
            score49ToTable(0,int(j),prec_df_rf_2)
            score49ToTable(0,int(j),recall_df_rf_2)

            # RF 3
            score49ToTable(0,int(j),score_df_rf_3)
            score49ToTable(0,int(j),f1_df_rf_3)
            score49ToTable(0,int(j),prec_df_rf_3)
            score49ToTable(0,int(j),recall_df_rf_3)


            # GBM 1
            score49ToTable(0,int(j),score_df_gbm_1)
            score49ToTable(0,int(j),f1_df_gbm_1)
            score49ToTable(0,int(j),prec_df_gbm_1)
            score49ToTable(0,int(j),recall_df_gbm_1)

            # GBM 2
            score49ToTable(0,int(j),score_df_gbm_2)
            score49ToTable(0,int(j),f1_df_gbm_2)
            score49ToTable(0,int(j),prec_df_gbm_2)
            score49ToTable(0,int(j),recall_df_gbm_2)

            # GBM 3
            score49ToTable(0,int(j),score_df_gbm_3)
            score49ToTable(0,int(j),f1_df_gbm_3)
            score49ToTable(0,int(j),prec_df_gbm_3)
            score49ToTable(0,int(j),recall_df_gbm_3)
        else:
            print(f'{j}:')
            writer1.write(f'\n{j}: ')
            writer2.write(f'\n{j}:')
...
def getSceneVelList():
    pos_list = getScenePosList()
    vel_list = []
    for i in range(len(pos_list)):
        vel_list.append([element + "Dt" for element in pos_list[i]])
    return vel_list
...
def getHeadPosList():
    ...
    head_pos = []
    head_euler = ["".join(c) for c in product(head,euler)]
    head_euler.remove('Head_euler_y') 
    head_quat = ["".join(c) for c in product(head,quat)]
    head_sixd = ["".join(c) for c in product(head,sixD)]
    right_pos = ["".join(c) for c in product(rhand,position)]
    right_euler = ["".join(c) for c in product(rhand,euler)]
    right_quat = ["".join(c) for c in product(rhand,quat)]
    right_sixd = ["".join(c) for c in product(rhand,sixD)]
    left_pos = ["".join(c) for c in product(lhand,position)]
    left_euler = ["".join(c) for c in product(lhand,euler)]
    left_quat = ["".join(c) for c in product(lhand,quat)]
    left_sixd = ["".join(c) for c in product(lhand,sixD)]

    _0 = head_pos + head_euler + right_pos + right_euler + left_pos + left_euler
    _1 = head_pos + head_quat + right_pos + right_quat + left_pos + left_quat
    _2 = head_pos + head_sixd + right_pos + right_sixd + left_pos + left_sixd
    # _3 = head_pos + right_pos + left_pos
    _4 = head_euler + right_euler + left_euler
    _5 = head_quat + right_quat + left_quat
    _6 = head_sixd + right_sixd + left_sixd

    _7 = head_pos + head_euler + right_pos + right_euler
    _8 = head_pos + head_quat + right_pos + right_quat
    _9 = head_pos + head_sixd + right_pos + right_sixd
    # _10 = head_pos + right_pos
    _11 = head_euler + right_euler
    _12 = head_quat + right_quat
    _13 = head_sixd + right_sixd

    _14 = head_pos + head_euler + left_pos + left_euler
    _15 = head_pos + head_quat + left_pos + left_quat
    _16 = head_pos + head_sixd + left_pos + left_sixd
    # _17 = head_pos + left_pos
    _18 = head_euler + left_euler
    _19 = head_quat + left_quat
    _20 = head_sixd + left_sixd

    _21 = right_pos + right_euler + left_pos + left_euler
    _22 = right_pos + right_quat + left_pos + left_quat
    _23 = right_pos + right_sixd + left_pos + left_sixd
    _24 = right_pos + left_pos
    _25 = right_euler + left_euler
    _26 = right_quat + left_quat
    _27 = right_sixd + left_sixd

    # _28 = head_pos + head_euler
    # _29 = head_pos + head_quat 
    # _30 = head_pos + head_sixd 
    # _31 = head_pos
    _32 = head_euler 
    _33 = head_quat 
    _34 = head_sixd 

    _35 = right_pos + right_euler
    _36 = right_pos + right_quat 
    _37 = right_pos + right_sixd 
    _38 = right_pos 
    _39 = right_euler
    _40 = right_quat
    _41 = right_sixd

    _42 = left_pos + left_euler
    _43 = left_pos + left_quat
    _44 = left_pos + left_sixd
    _45 = left_pos
    _46 = left_euler
    _47 = left_quat
    _48 = left_sixd

    body_relative_list = [_0,_1,_2,[],_4,_5,_6,_7,_8,_9,[],_11,_12,_13,_14,_15,_16,[],_18,_19,_20,_21,_22,_23,_24,_25,_26,_27,[],[],[],[],_32,_33,_34,_35,_36,_37,_38,_39,_40,_41,_42,_43,_44,_45,_46,_47,_48]
    
    return body_relative_list

def getHeadVelList():
    pos_list = getHeadPosList()
    vel_list = []
    for i in range(len(pos_list)):
        vel_list.append([element + "Dt" for element in pos_list[i]])
    return vel_list
...
def savePredictionDf(pred_df,model,experimentnum,experiment_info,parentfolder):
    ...
    outputdir = f"{documents_filepath}\\{parentfolder}\\{experiment_info['train_info']}-{experiment_info['test_info']}_{experiment_info['relative_info']}_{experiment_info['posorvel_info']}_{experiment_info['subsample_info']}_PredDf"
    ...
    filename = f"{experiment_info['train_info']}-{experiment_info['test_info']}_{experiment_info['relative_info']}_{experiment_info['posorvel_info']}_{experiment_info['subsample_info']}_{model}_{experimentnum}.csv"    
...
def createScoreTable(model,experiment_info,type):
    ...
    if (experiment_info['subsample_info'] != ""):
        df.index.name = f"{type}_{experiment_info['train_info']}-{experiment_info['test_info']}_{model}_{experiment_info['relative_info']}_{experiment_info['posorvel_info']}_{experiment_info['subsample_info']}"
    else:
        df.index.name = f"{type}_{experiment_info['train_info']}-{experiment_info['test_info']}_{model}_{experiment_info['relative_info']}_{experiment_info['posorvel_info']}"
